...
const AvailablePlayers = ({ handlePlayers, isActive, coin, setCoin }) => {
  ...
  const handleSelectedPlayers = (player) => {
    ...
    if (player.price > coin) {
      toast.error("Oops! You don’t have enough coins");
    } else if (selectedPlayers.length >= 6) {
      toast.error("Team limit reached! You can't add more than 6 players.");
    } else if (soldPlayers) {
      toast.error("Already choosen !");
    } else {
      setCoin(coin - player.price);
      setSelectedPlayers([...selectedPlayers, player]);
      toast.success(`${player.name} is choosen by you`);
    }
  };
      ...
      <div className="flex justify-between items-center mb-12 pt-12">
        ...
        <div className="border rounded-2xl">
          <button
            ...
            className={`${isActive.isActive ? "active" : "py-4 px-8 text-gray-600"
              }`}
          >
            Available
          </button>
          <button
            ...
            className={`${isActive.isActive ? "py-4 px-8 text-gray-600" : "active"
              }`}
          >
